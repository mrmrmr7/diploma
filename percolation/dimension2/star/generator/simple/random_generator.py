...
class Generator:
    ...
    def generate_stars(self, count):
        stars = []
        for i in range(self.p["star_count"]):
            stars.append(
                {
                    "phi_0": random.randint(0, 180),
                    "r_inner": self.p["r_inner"],
                    "r_outer": self.p["r_outer"],
                    "x": random.randint(0, self.p["x_max"]),
                    "y": random.randint(0, self.p["y_max"])
                }
            )
        return stars

    def solve_intersections(self, _stars):
        s_intersect = {}
        for s1 in _stars:
            s_intersect_list = []

            for s2 in _stars:
                if s1 != s2 and self.is_stars_intersect(s1, s2):
    ...
    def impose_stars_on_figure(self, _stars, _p, _get_scatter):
        fig = go.Figure(
            layout=go.Layout( 
                width=_p["layout_w"],
                height=_p["layout_h"],
    ...
    def generate_with_star_count(self, star_count, r_inner, r_outer):
        self.p={
        ...
        stars = self.generate_stars(self.p)
        stars_without_intersections = self.solve_intersections(stars)
        
        parsed = [ json.loads(each) for each in stars_without_intersections ]
