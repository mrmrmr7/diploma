...
class Visualizer:
    def generate_accurate_square_points(self, _star, accuracy=3):
        (x_arr, y_arr) = self.generate_square_points(_star)
    ...
    def get_square_scatter(self, _star, _type="simple", accuracy=3):
        x_arr = []
        y_arr = []
        if _type == "simple":
            (x_arr, y_arr) = _star.generate_square_points(_star)
        if _type == "accurate":
            (x_arr, y_arr) = _star.generate_accurate_square_points(_star, accuracy)
        return go.Scatter(
            x=x_arr,
            y=y_arr,
    ...
    def vizualize(self, _stars, axis_size):
        _p = {
            "a": _stars[0].a,
            "x_max": axis_size,
            "y_max": axis_size,
        ...
        fig = go.Figure(
            layout=go.Layout(
                width=_p["layout_w"],
                height=_p["layout_h"],
                xaxis={"range": [-_p["a"], _p["x_max"]+_p["a"]]},
                yaxis={"range": [-_p["a"], _p["y_max"]+_p["a"]]},
        ...
        fig.update_yaxes(showline=True, linewidth=10,
                         linecolor='black', mirror=True)
        for each in _stars:
            fig.add_trace(self.get_square_scatter(each))
